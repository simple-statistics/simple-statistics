require("../test/add_to_mean.test.js");
require("../test/approx_equal.test.js");
require("../test/bayes.test.js");
require("../test/bernoulli_distribution.test.js");
require("../test/binomial_distribution.test.js");
require("../test/bisect.test.js");
require("../test/chi_squared_goodness_of_fit.test.js");
require("../test/chunk.test.js");
require("../test/ckmeans.test.js");
require("../test/coefficient_of_variation.test.js");
require("../test/combinations.test.js");
require("../test/combinations_replacement.test.js");
require("../test/combine_means.test.js");
require("../test/combine_variances.test.js");
require("../test/cumulative.test.js");
require("../test/equal_interval_breaks.test.js");
require("../test/error_function.test.js");
require("../test/factorial.test.js");
require("../test/gamma.test.js");
require("../test/gammaln.test.js");
require("../test/geometric_mean.test.js");
require("../test/harmonic_mean.test.js");
require("../test/iqr.test.js");
require("../test/jenks.test.js");
require("../test/k_means_cluster.test.js");
require("../test/kernel_density_estimation.test.js");
require("../test/linear_regression.test.js");
require("../test/log_average.test.js");
require("../test/logit.test.js");
require("../test/mad.test.js");
require("../test/mean.test.js");
require("../test/mean_simple.test.js");
require("../test/median.test.js");
require("../test/minmax.test.js");
require("../test/mode.test.js");
require("../test/normal_distribution.test.js");
require("../test/numeric_sort.test.js");
require("../test/perceptron.test.js");
require("../test/permutation_test.test.js");
require("../test/permutations_heap.test.js");
require("../test/poisson_distribution.test.js");
require("../test/product.test.js");
require("../test/quantile.test.js");
require("../test/quantile_rank.test.js");
require("../test/quantile_rank_sorted.test.js");
require("../test/quantilesorted.test.js");
require("../test/quickselect.test.js");
require("../test/r_squared.test.js");
require("../test/relative_error.test.js");
require("../test/root_mean_square.test.js");
require("../test/sample.test.js");
require("../test/sample_correlation.test.js");
require("../test/sample_covariance.test.js");
require("../test/sample_kurtosis.test.js");
require("../test/sample_skewness.test.js");
require("../test/sample_standard_deviation.test.js");
require("../test/sample_variance.test.js");
require("../test/sample_with_replacement.test.js");
require("../test/shuffle.test.js");
require("../test/sign.test.js");
require("../test/silhouette.test.js");
require("../test/standard_deviation.test.js");
require("../test/standard_normal_table.test.js");
require("../test/subtract_from_mean.test.js");
require("../test/sum.test.js");
require("../test/sum_nth_power_deviations.test.js");
require("../test/t_test.test.js");
require("../test/t_test_two_sample.test.js");
require("../test/types.js");
require("../test/types.ts");
require("../test/unique_count_sorted.test.js");
require("../test/variance.test.js");
require("../test/weighted_mean.test.js");
require("../test/weighted_quantile.test.js");
require("../test/weighted_standard_deviation.test.js");
require("../test/weighted_variance.test.js");
require("../test/wilcoxon_rank_sum.test.js");
require("../test/z_score.test.js");