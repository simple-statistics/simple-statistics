...
test("approximate strict equality", function (t) {
    ...
    t.test("handles negative values", function (t) {
        t.ok(ss.approxEqual(-10, -10));
        t.ok(ss.approxEqual(-10 - EPS, -10));
        t.ok(!ss.approxEqual(-10 - 11 * EPS, -10));
        t.ok(!ss.approxEqual(-10, 10));
        t.end();
    });

    t.test("handles larger tolerances", function (t) {
        t.ok(!ss.approxEqual(1, 2));
        t.ok(ss.approxEqual(1, 2, 1.5));
        t.end();
    });

    t.test("handles values near zero", function (t) {
        t.ok(!ss.approxEqual(1, 0));
        t.ok(!ss.approxEqual(0, 1));
        t.end();
    });
