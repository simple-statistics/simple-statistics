...
test("quantileSorted", function (t) {
    ...
    t.test("can get proper quantiles of an even-length list", function (t) {
        ...
        t.equal(ss.quantileSorted(even, 0.25), 7.25);
        t.equal(ss.quantileSorted(even, 0.5), 9);
        t.equal(ss.quantileSorted(even, 0.75), 14.5);
        t.end();
    });

    t.test("can get proper quantiles of an odd-length list", function (t) {
        const odd = Object.freeze([3, 6, 7, 8, 8, 9, 10, 13, 15, 16, 20]);
        t.equal(ss.quantileSorted(odd, 0.25), 7.5);
        t.equal(ss.quantileSorted(odd, 0.5), 9);
        t.equal(ss.quantileSorted(odd, 0.75), 14);
        t.end();
    });
    t.end();
});
